...
class ConservativeSAC(object):
    ...
    def get_default_config(updates=None):
        ...
        config.cql_target_action_gap = 1.0 #this is only used if we are using cql_lagrange
        config.cql_temp = 1.0
    ...
    def update_terminal_info_from_data_batch(self, dones):
        done_list = list()
        for v in list(dones):
            v = 0
            if v:
